...
bool MyStl::Node::unpoint(const Node &node)
{
...
bool MyStl::Node::unpointLastVisit(Graph &graph)
{
...
bool MyStl::Node::disconnect(Node &node)
{
    for (Neighbour &neighbour : this->neighbours)
    {
        if (neighbour.nodeId == node.nodeId)
...
bool MyStl::Node::disconnectLastVisit(Graph &graph)
{
    if (this->lastVisited().nodeId == MyStl::Node::UNDEFINED_NODE)
        return false;
...
MyStl::Node::Neighbour MyStl::Node::move(Graph &graph)
{
...
MyStl::Node::Neighbour MyStl::Node::moveUnvisited(Graph &graph)
{
...
uint32_t MyStl::Node::neighbourCount() const
{
    return this->neighbours.size();
}
const std::vector<MyStl::Node::Neighbour> &MyStl::Node::get_neighbours() const
{
...
void MyStl::Node::priortiseNeighbourByLowWeight()
{
    std::sort(this->neighbours.begin(), this->neighbours.end(), [](Neighbour neighbour1, Neighbour neighbour2)
              { return neighbour1.weight < neighbour2.weight; });
}
void MyStl::Node::priortiseNeighbourByHeighNodeId()
{
    std::sort(this->neighbours.begin(), this->neighbours.end(), [](Neighbour neighbour1, Neighbour neighbour2)
              { return neighbour1.nodeId > neighbour2.nodeId; });
}
void MyStl::Node::priortiseNeighbourByLowNodeId()
{
    std::sort(this->neighbours.begin(), this->neighbours.end(), [](Neighbour neighbour1, Neighbour neighbour2)
              { return neighbour1.nodeId < neighbour2.nodeId; });
}
...
void MyStl::Graph::priortiseNeighbourByHeighWeight()
{
...
void MyStl::Graph::priortiseNeighbourByLowWeight()
{
    for (MyStl::Node &node : this->graph)
    {
...
void MyStl::Graph::priortiseNeighbourByHeighNodeId()
{
    for (MyStl::Node &node : this->graph)
    {
...
void MyStl::Graph::priortiseNeighbourByLowNodeId()
{
    for (MyStl::Node &node : this->graph)
    {
...
void MyStl::Graph::inputEdges(uint32_t numberOfEdges)
{
...
void MyStl::Graph::inputDirectedEdges(uint32_t numberOfEdges)
{
    for (uint32_t edgeNo = 0; edgeNo < numberOfEdges; edgeNo++)
    {
        MyStl::Node::NodeId node1, node2;
...
MyStl::Node::NodeId MyStl::Graph::unvisitedNode()
{
